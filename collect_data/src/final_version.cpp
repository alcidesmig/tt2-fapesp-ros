// Possível #TODO: serviço para calcular o true airspeed
...
// Acompanhar chegada nos waypoints
...
// Dados de pressão ambiente para cálculo do "true airspeed"
...
int alt_point = -1, value_quaternion, collecting, cont_spin, collecting_2_point = 0,
    collecting_3_point = 0, last_waypoint = -1, status = -1, can_compare_for_loop = 0,
    try_sync_done = 0, lidar_ok = 0, lidar_sincronized = 0, change_z = 0;

double value_quat2 = 0.0, const_sum_quat = 0.01745329251, // 2*pi / 360
       offboard_enabled = -1, compass_diff = 0, yaw_compass_start_value,
       max_airspeed[3] = { -1, -1, -1}, altitude = 0, zero_i_airspeed = -1.0, zero_t_airspeed = -1.0;
...
float alt_1_point = 2, alt_2_point = 5, alt_3_point = 10, pos_x = 0, pos_y = 0, pos_z = 0, offset_lidar_barometer = 0;

char data[3][256], data_file_name[128], file_name_regina[128];
...
// Calcular "true airspeed" -> https://subak.io/code/ardupilot/ArduCopter-3.5/modules/PX4Firmware/src/modules/systemlib/airspeed.c.html
...
// Servidor HTTP para fornecer dados em JSON
...
// Função para fazer o parse da string de dados separados por ; para JSON
...
// Thread para enviar dados para o drone em relação a rotação (quaternion),
// gravar os dados no arquivo bruto e na string para posterior gravação no arquivo "regina"
void *thread_func_set_pos(void *args)
{
	...
	// Referência de tempo para pegar os milésimos de segundo e utilizar junto com o timestamp do GPS                              
	ros::Subscriber time_reference_sub = nh.subscribe<sensor_msgs::TimeReference>
	                                     ("mavros/time_reference", 1, get_time_reference);

	ros::Rate rate_thread(72.0); // Valor: 72 utilizado de forma a causar a rotação no tempo desejado;
	// Consequentemente, por estar na mesma thread, causa com que os dados sejam escritos na mesma velocidade
	// caso seja necessária desacoplação, basta separar em duas threads (gravação de dados e envio de quaternion)
	// Possível #TODO
	...
	while (ros::ok())
	{
		if (collecting)
		{
			try
			{
				value_quat2 += const_sum_quat; // Valor que é adicionado a um rate de 72Hz no quaternion para rotação do drone, a 
				// estratégia utilizada foi adicionar em pequenas quantidades e realizar mini rotações
				// para implicar numa rotação inteira
				...
				if (fp != NULL)
				{
					...
					if (hdc1050.valid && ms4525.valid) // Se os dados estiverem válidos grava no arquivo
					{
						...
						// Se forem dados de pico, salva na string de dados no ponto máximo
						if (alt_point != -1 && true_airspeed > max_airspeed[alt_point - 1])
						{
...
int main(int argc, char **argv)
{
	...
	fp = fopen(data_file_name, "a+");
  	while(fp == NULL && !ms4525.valid); // Espera chegar algum dado do sensor de ms4525 para gravar o 0 do sensor de airspeed.
	sleep(5); // Para dados consistentes do sensor, popula a média móvel
	...
	fprintf(fp, "temperatura;raw indicated airspeed;indicated airspeed (-offset);raw true airspeedtrue airspeed (-offset);umidade;bússola;latitude;longitude;altitude gps;rel_alt;lidar;altitude calculada;timestamp;timestamp_nsecs(utilizar os décimos)\n");
	fclose(fp);
	...
	while (ros::ok())
	{
		switch (status) // Máquina de estados
		{
		case -1:
			// Verifica se chegou em um waypoint comparando o último que foi registrado com o atual. Obs: o primeiro é sempre descartado, por especificação.
			if (last_waypoint != waypoint_num.wp_seq && waypoint_num.wp_seq != 0)
			{
				...
				// Grava a chegada no waypoint no arquivo
		...
		case 0:
			offb_set_mode.request.custom_mode = "OFFBOARD";
			// Se não estiver no OFFBOARD -> tente colocar
			...
			// Faz a sincronização da altura do drone com o dado do LIDAR
			if (current_state.mode == "OFFBOARD" && !try_sync_done)
			{
				...
				// As três linhas abaixo eram utilizadas na estratégia anterior de descida do drone, onde ao invés de setar uma
				// velocidade de descida, era setado um ponto para o drone descer e alcanãr.
				...
				if (lidar_ok && !lidar_sincronized) // Se o lidar estiver ok (já tiver registrado algum valor válido) e a altura não foi sincronizada (offset calculado), faz a sincronização
				{
					...
					// Manda o drone para baixo até que atinja uma altura na qual o lidar é valido
					if (pos.pose.position.z > 7 && (lidar.range < 1 || lidar.range > 7)) 
					{
					...
					if (lidar.range > 3 && lidar.range < 7) // Se entrou num range válido do lidar, faz o cálculo do offset e manda o drone para a primeira altura de coleta
					{
						...
						lidar_sincronized = 1;
						// Manda o drone para cima ou para baixo, a depender da posição relativa em relação à altura de coleta
						if (rel_alt.data - offset_lidar_barometer > alt_1_point) 
						{
							pos_target.velocity.z = -0.5 * 2;
						}
						else
			...
			if (current_state.mode == "OFFBOARD" && try_sync_done)
			{
				...
				// segunda condição: ter batido 2*PI adicionados no quaternion (= 360 graus e, portanto, uma rotação)
				if (collecting && ((can_compare_for_loop && diff(yaw_compass_start_value, compass.data) < 3) || value_quat2 > 6.28318530718) )
				{
					...
					if (collecting_3_point) // Se coletou dados no 3 ponto de coleta
					{
						...
						fp = NULL;
						alt_point = -1;
						// Grava dados de pico de cada altura no arquivo "regina"
						...
						// Prepara dados para envio via HTTP
					...
					else if (!collecting_2_point && !collecting_3_point && status != 1)
					{
						...
						{
							pos_target.velocity.z = -0.5 * 2; // Velocidade de subida/descida
						}
						else
						{
							pos_target.velocity.z = 0.5 * 2;
		...
		if (change_z) // Caso seja para publicar o pos_target (momento de descida)
		{			  // Sincroniza os valores de x e y
		...
		else // Sincroniza os valores
		{
			pose.pose.position.x = pos_x; 
			pose.pose.position.y = pos_y;
			pose.pose.position.z = pos_z;
		}
		// Faz a verificação constante para ver se em algum momento o lidar coletou valores válidos
		if (!lidar_ok && lidar.range > 2 && lidar.range < 8) 
		{
			lidar_ok = 1;
		}
